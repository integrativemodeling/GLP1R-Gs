...
#---MODELLER SECTION---#####
    ...
    cmp_key = 'DOPE score'
    keyfunc = functools.cmp_to_key(lambda x, y: cmp(x[cmp_key], y[cmp_key]))
...
for cluster in clusters:
    for sample in samples:
        pdbdir = top_dir+'/'+cluster+'/'+sample+'/'
        pdb_files = glob.glob(pdbdir + '*'+ str(pdbID) +'.pdb')
        for pdb_file in pdb_files:
            file_name = re.search(pdbdir + '(.+?).pdb', pdb_file)
            seq = file_name.group(1)
            print(seq)
            out_name = cluster+ '_' + seq + '_all.pdb'
            ca2all(pdb_file, output=out_name, iterations=1)  
